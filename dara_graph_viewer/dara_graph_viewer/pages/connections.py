...
from dara.components import Button, Stack, Tab, TabbedCard, Text
from dara.core import DerivedVariable, UpdateVariable, ResetVariables, Variable, py_component
...
def ConnectionsPage():
    ...
    transitivity_var = DerivedVariable(
        calculate_transitivity_index, variables=[graph_var],
    )
        ...
        Tab(
            Stack(
                ...
                Stack(
                    ...
                    Text('The Transitivity Index demonstrates how well your graph is connected out of its full connectivity potential. It is the ratio transitive triads to potential transitive triads.'),
                    Text('Try adding or deleting edges to see how it would affect the Transitivity Index of your social network.'),
                    DisplayIndex(transitivity_var),
                    Text('In the next tab, you can see recommended connections for each individual and how those connections would boos the Transitivity Index of your network.'),
                    Stack(
                        Button('Reset Graph', onclick=ResetVariables(variables=[graph_var])),
                        justify='end',
                    ),
        ...
        Tab(
            Stack(
                CausalGraphViewer(
                    causal_graph=graph_var,
                    editor_mode=EditorMode.PAG,
                    on_click_node=UpdateVariable(lambda ctx: ctx.inputs.new['identifier'], variable=selected_node_var),
                ),
                Stack(
                    Text('Connection Recommendations', font_size='1.2rem'),
                    Text('Select a node to see an individual\'s recommended connections based on a naive triad analysis.'),             
